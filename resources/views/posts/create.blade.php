...
@section('content')
    ...
    <div style="max-width: 600px; margin: 0 auto; background: #f9f9f9; padding: 20px; border-radius: 10px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.1);">
        <form action="{{ route('posts.store') }}" method="POST" style="display: flex; flex-direction: column; gap: 15px;">
            @csrf
            <label style="font-weight: bold;">Title</label>
            <input type="text" name="title" required style="padding: 10px; border: 1px solid #ccc; border-radius: 5px; width: 100%;">

            <label style="font-weight: bold;">Content</label>
            <textarea name="content" required rows="5" style="padding: 10px; border: 1px solid #ccc; border-radius: 5px; width: 100%; resize: vertical;"></textarea>
